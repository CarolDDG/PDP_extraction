#!/usr/bin/env python3
"""
LoRA Fine-Tuning entry point for cosmetic-poster OCR with GLM-OCR.
Includes WSL2 memory patch, trust_remote_code, and hardware resource monitoring.
"""
...
@dataclass
class GLMOCRTrainConfig:
    train_file: str
    image_root: str
    output_dir: str = "outputs/glm-ocr-cosmetic-ocr-lora"
...
def train_glm_ocr(config: GLMOCRTrainConfig) -> dict[str, float]:
    """Fine-tune GLM-OCR for cosmetic poster OCR."""
    ...
    try:
        processor = AutoProcessor.from_pretrained(
            config.model_name, max_pixels=config.max_pixels, trust_remote_code=True
        )
    except Exception:
...
def _parse_args() -> GLMOCRTrainConfig:
    ...
    parser.add_argument("--output-dir", default="outputs/glm-ocr-cosmetic-ocr-lora")
    parser.add_argument("--eval-file", help="Path to eval_manifest.json")
    parser.add_argument("--model-name", default="zai-org/GLM-OCR")
    ...
    return GLMOCRTrainConfig(**vars(args))


if __name__ == "__main__":
    metrics = train_glm_ocr(_parse_args())
    print("=" * 60)
    print("🎉 GLM-OCR 微调已完成！统计指标与硬件资源负载如下:")
